...
class RepositoryModule(
    ...
    suspend fun fetchMovieGenres(): List<Genre> {
        return try {
            val response = apiService.getMovieGenres(token = accessToken)
            response.genres.map {
                Genre(
                    id = it.id,
                    name = it.name
    ...
    suspend fun fetchTvShowGenres(): List<Genre> {
        return try {
            val response = apiService.getTvShowGenres(token = accessToken)
            response.genres.map {
                Genre(
                    id = it.id,
                    name = it.name
    ...
    suspend fun fetchMoviesByGenre(genreId: Int): List<Movie> {
        return try {
            val response = apiService.getMoviesByGenre(genreId = genreId, token = accessToken)
            response.results.map { movie ->
                Movie(
                    id = movie.id,
                    title = movie.title,
    ...
    suspend fun fetchTvShowsByGenre(genreId: Int): List<TvShow> {
        return try {
            val response = apiService.getTvShowsByGenre(genreId = genreId, token = accessToken)
            response.results.map { tvShow ->
                TvShow(
                    id = tvShow.id,
                    name = tvShow.name,
